...
/// Anime4K 超分辨率设置区块
Widget _buildAnime4KSection(BuildContext context) {
  final EhSettingService ehSettingService = Get.find();
  ...
  return Obx(() {
    ...
    if (!ehSettingService.enableAnime4K) {
      footerText = '关闭状态：图片以原始分辨率显示。';
    } else {
      final scale = ehSettingService.anime4KScaleFactor.toStringAsFixed(1);
      final networkNote = ehSettingService.enableAnime4KForNetwork
          ? '本地图片和网络图片均已开启超分。'
          : '仅对已下载的本地图片超分，网络图片不进行处理。';
      footerText = '已启用：图片将在后台使用 Anime4K 算法进行 ${scale}x 超分辨率处理。处理结果将被缓存。$networkNote';
    }

    return SliverCupertinoListSection.listInsetGrouped(
      header: const Text('Anime4K 超分辨率'),
      footer: Text(footerText),
      children: [
        // 本地图片超分开关
        EhCupertinoListTile(
          title: const Text('启用 Anime4K 超分辨率'),
          subtitle: const Text('对已下载的本地图片启用超分'),
          trailing: CupertinoSwitch(
            value: ehSettingService.enableAnime4K,
            onChanged: (val) {
              ehSettingService.enableAnime4K = val;
            },
          ),
        ),
        // 网络图片超分开关（仅在本地超分开关开启时显示）
        if (ehSettingService.enableAnime4K)
          EhCupertinoListTile(
            title: const Text('网络图片也进行超分'),
            subtitle: const Text('对在线浏览的网络图片同样进行 Anime4K 超分处理'),
            trailing: CupertinoSwitch(
              value: ehSettingService.enableAnime4KForNetwork,
              onChanged: (val) {
                ehSettingService.enableAnime4KForNetwork = val;
              },
            ),
          ),
        // 放大倍数和其他参数（仅在启用时显示）
        if (ehSettingService.enableAnime4K) ..._buildAnime4KOptions(context, ehSettingService),
      ],
    );
  });
}
...
/// Anime4K 详细参数选项
List<Widget> _buildAnime4KOptions(BuildContext context, EhSettingService ehSettingService) {
  return [
    // 放大倍数
    EhCupertinoListTile(
      title: const Text('放大倍数'),
      additionalInfo: Obx(() => Text(
        '${ehSettingService.anime4KScaleFactor.toStringAsFixed(1)}x',
      )),
      trailing: const CupertinoListTileChevron(),
      onTap: () async {
        final Map<double, String> scaleMap = {
          1.5: '1.5x（快速）',
          2.0: '2.0x（推荐）',
          3.0: '3.0x（高质量）',
          4.0: '4.0x（极高质量）',
        ...
        final result = await showCupertinoModalPopup<double>(
          ...
          builder: (ctx) => CupertinoActionSheet(
            title: const Text('选择放大倍数'),
            cancelButton: CupertinoActionSheetAction(
              onPressed: () => Get.back(),
              child: Text(L10n.of(context).cancel),
            ),
            actions: scaleMap.entries
                .map((e) => CupertinoActionSheetAction(
                      onPressed: () => Get.back(result: e.key),
                      isDefaultAction: ehSettingService.anime4KScaleFactor == e.key,
                      child: Text(e.value),
                    ))
                .toList(),
          ),
        ...
        if (result != null) {
          ehSettingService.anime4KScaleFactor = result;
        }
      },
    ),
    // 线条细化强度
    EhCupertinoListTile(
      title: const Text('线条细化强度'),
      additionalInfo: Obx(() => Text(
        ehSettingService.anime4KPushStrength.toStringAsFixed(2),
      )),
      trailing: const CupertinoListTileChevron(),
      onTap: () async {
        final Map<double, String> strengthMap = {
          0.0: '0.00（关闭）',
          0.15: '0.15（轻微）',
          0.31: '0.31（推荐）',
          0.50: '0.50（强烈）',
        ...
        final result = await showCupertinoModalPopup<double>(
          context: context,
          builder: (ctx) => CupertinoActionSheet(
            title: const Text('选择线条细化强度'),
            cancelButton: CupertinoActionSheetAction(
              onPressed: () => Get.back(),
              child: Text(L10n.of(context).cancel),
            ),
            actions: strengthMap.entries
                .map((e) => CupertinoActionSheetAction(
                      onPressed: () => Get.back(result: e.key),
                      isDefaultAction: ehSettingService.anime4KPushStrength == e.key,
                      child: Text(e.value),
                    ))
                .toList(),
          ),
        ...
        if (result != null) {
          ehSettingService.anime4KPushStrength = result;
        }
      },
    ),
    // 梯度精炼强度
    EhCupertinoListTile(
      title: const Text('梯度精炼强度'),
      additionalInfo: Obx(() => Text(
        ehSettingService.anime4KPushGradStrength.toStringAsFixed(2),
      )),
      trailing: const CupertinoListTileChevron(),
      onTap: () async {
        final Map<double, String> gradMap = {
          0.0: '0.00（关闭）',
          0.5: '0.50（轻微）',
          1.0: '1.00（推荐）',
          1.5: '1.50（强烈）',
        ...
        final result = await showCupertinoModalPopup<double>(
          context: context,
          builder: (ctx) => CupertinoActionSheet(
            title: const Text('选择梯度精炼强度'),
            cancelButton: CupertinoActionSheetAction(
              onPressed: () => Get.back(),
              child: Text(L10n.of(context).cancel),
            ),
            actions: gradMap.entries
                .map((e) => CupertinoActionSheetAction(
                      onPressed: () => Get.back(result: e.key),
                      isDefaultAction: ehSettingService.anime4KPushGradStrength == e.key,
                      child: Text(e.value),
                    ))
                .toList(),
          ),
        ...
        if (result != null) {
          ehSettingService.anime4KPushGradStrength = result;
        }
      },
    ),
  ];
