...
class EhSettingService extends ProfileService {
  ...
  // enableAnime4K - 是否启用 Anime4K 超分辨率
  final _enableAnime4K = false.obs;
  bool get enableAnime4K => _enableAnime4K.value;
  set enableAnime4K(bool val) => _enableAnime4K.value = val;

  // anime4KScaleFactor - 超分放大倍数
  final _anime4KScaleFactor = 2.0.obs;
  double get anime4KScaleFactor => _anime4KScaleFactor.value;
  set anime4KScaleFactor(double val) => _anime4KScaleFactor.value = val;

  // anime4KPushStrength - 线条细化强度
  final _anime4KPushStrength = 0.31.obs;
  double get anime4KPushStrength => _anime4KPushStrength.value;
  set anime4KPushStrength(double val) => _anime4KPushStrength.value = val;

  // anime4KPushGradStrength - 梯度精炼强度
  final _anime4KPushGradStrength = 1.0.obs;
  double get anime4KPushGradStrength => _anime4KPushGradStrength.value;
  set anime4KPushGradStrength(double val) => _anime4KPushGradStrength.value = val;

  // enableAnime4KForNetwork - 是否对网络图片启用 Anime4K 超分辨率
  final _enableAnime4KForNetwork = false.obs;
  bool get enableAnime4KForNetwork => _enableAnime4KForNetwork.value;
  set enableAnime4KForNetwork(bool val) => _enableAnime4KForNetwork.value = val;
  ...
  void _initEhConfig() {
    ...
    // _enableSlideOutPage
    enableSlideOutPage = ehConfig.enableSlideOutPage ?? enableSlideOutPage;
    everProfile<bool>(_enableSlideOutPage, (value) {
      ehConfig = ehConfig.copyWith(enableSlideOutPage: value.oN);
    });

    // enableAnime4K
    enableAnime4K = ehConfig.enableAnime4K ?? enableAnime4K;
    everProfile<bool>(_enableAnime4K, (value) {
      ehConfig = ehConfig.copyWith(enableAnime4K: value.oN);
    });

    // anime4KScaleFactor
    anime4KScaleFactor = ehConfig.anime4KScaleFactor ?? anime4KScaleFactor;
    everProfile<double>(_anime4KScaleFactor, (value) {
      ehConfig = ehConfig.copyWith(anime4KScaleFactor: value.oN);
    });

    // anime4KPushStrength
    anime4KPushStrength = ehConfig.anime4KPushStrength ?? anime4KPushStrength;
    everProfile<double>(_anime4KPushStrength, (value) {
      ehConfig = ehConfig.copyWith(anime4KPushStrength: value.oN);
    });

    // anime4KPushGradStrength
    anime4KPushGradStrength =
        ehConfig.anime4KPushGradStrength ?? anime4KPushGradStrength;
    everProfile<double>(_anime4KPushGradStrength, (value) {
      ehConfig = ehConfig.copyWith(anime4KPushGradStrength: value.oN);
    });

    // enableAnime4KForNetwork
    enableAnime4KForNetwork =
        ehConfig.enableAnime4KForNetwork ?? enableAnime4KForNetwork;
    everProfile<bool>(_enableAnime4KForNetwork, (value) {
      ehConfig = ehConfig.copyWith(enableAnime4KForNetwork: value.oN);
    });
  }
